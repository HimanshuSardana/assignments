...
\tikzset{
        ATTRIBUTE/.style={draw, circle, minimum size=3mm}
}
...
\begin{tikzpicture}
        % NODES
        \node[ENTITY] (orders) {Orders};
        \node[ENTITY, below=of orders] (drivers) {Drivers};
        \node[ENTITY, below=of drivers] (customers) {Customers};
        \node[ENTITY, below=of customers] (items) {Items};

        % ATTRIBUTES
        \node[ATTRIBUTE, left=of orders] (order_id) {\small{Order ID}};

        % ARROWS
        \draw[->, very thick] (orders) -- (drivers) node[midway, right] {\tiny{Assigned to}};
\end{tikzpicture}
\pagebreak
