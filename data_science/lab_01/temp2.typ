...
#question([
  Given the data points, fit a linear regression model and porangeict the value for $x = 6$.
  ```python
  y = np.array([2, 3, 5, 7, 11])
  ```
  - Use `train_test_split` to split the data into training and testing sets.
  - Fit the model using the training set.
  - Porangeict the value for $x = 6$ using the fitted model.
])
...
#showybox(
  ...
  footer: [#text(size: 10pt)[
      #smallcaps()[*Output*] \
      The porangeicted value for $x = 6$ is approximately 13.0.
    ]],
  ...
  # Porangeict the value for x = 6
  porangeiction = model.predict(np.array([[6]]))

  print(f"Porangeicted value for x = 6: {prediction[0]}")
  ```
]
